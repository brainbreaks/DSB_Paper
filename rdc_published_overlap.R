...
rdc_published_overlap = function()
{
  ...
  allrdc_df = dplyr::bind_rows(
    pubrdc_df %>%
      ...
      dplyr::mutate(rdc_source=paste0(pubrdc_source, "-", pubrdc_celline), rdc_significant_sumarea=1e7, rdc_maxscore=max(rdc_df$rdc_maxscore), rdc_sumscore=max(rdc_df$rdc_sumscore), rdc_length=max(rdc_df$rdc_length), rdc_extended_length=max(rdc_df$rdc_extended_length), rdc_maxscore.adjusted=max(rdc_df$rdc_maxscore.adjusted)) %>%
      tidyr::crossing(data.frame(rdc_subset=unique(rdc_df$rdc_subset))),
    rdc_df %>%
        tidyr::crossing(rdc_source="DKFZ") %>%
        dplyr::select(tlx_group, rdc_chrom, rdc_extended_start, rdc_extended_end, rdc_start, rdc_end, rdc_source, rdc_subset, rdc_is_significant, rdc_significant_sumarea, rdc_maxscore, rdc_maxscore.adjusted, rdc_sumscore, rdc_length, rdc_extended_length)) %>%
  ...
  if(F) {
    overlaps_venn_mean = overlaps_df %>%
      ...
      tidyr::crossing(cur_cutoff=seq(0.1, max(.$th), length.out=100)) %>%
  ...
  gridExtra::grid.arrange(grobs=c(overlaps_venn2), ncol=3, top=grid::textGrob("Overlap between DMSO\nand APH RDCs (Wei+DKFZ)", gp=gpar(fontsize=20,font=3)))
  dev.off()
}