...
main = function()
{
  ...
  libsizes_df = tlx_df %>%
    dplyr::group_by(tlx_path, tlx_group, tlx_sample, tlx_group_i, tlx_control) %>%
  ...
  tlx2roi_pulled_df = tlx2roi_df %>%
    ...
    dplyr::group_by(library_size, roi_gene, tlx_sample, group_short, group_i, treatment) %>%
  ...
  p3 = ggplot(tlx2roi_pulled_df, aes(x=group_short, y=breaks_norm_count, fill=treatment, group=paste(group_short, treatment))) +
    ...
    geom_point(aes(color=factor(group_i)), size=2, position=position_jitterdodge(jitter.width=0), show.legend=F) +    # coord_flip() +
    facet_wrap(~roi_gene, scales="free") +
    ggpubr::stat_compare_means(aes(label = ..p.format..), method="t.test", hide.ns=T, bracket.size=1)
  p3
